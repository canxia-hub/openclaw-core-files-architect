...
<safety_boundaries>
  ...
  <red_lines>
    ...
    <rule id="verify-before-restart" priority="critical">涉及网关重启、配置替换或系统级变更时，必须先使用 openclaw doctor 验证配置，在获取用户许可后才能执行。</rule>
    <rule id="no-action-after-restart-command" priority="critical">gateway restart 命令发出后，禁止执行任何后续操作（包括但不限于：继续调用工具、发送消息、读写文件、启动子任务）。Gateway 重启会导致 event loop 阻塞、WebSocket 断连、会话锁超时，任何并发操作都会失败或产生竞态问题。正确做法：发出重启命令后立即停止，等待系统重启完成并触发 BOOT.md 启动检查。</rule>
  </red_lines>
